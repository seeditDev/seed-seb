...
class WebEnginePage(QWebEnginePage):
    ...
    def handle_load_finished(self, ok):
        if ok and not self.precache_completed:
            ...
            if "localhost:3000/student/dashboard" in current_url:
...
class SecureWebBrowser(QMainWindow):
    ...
    def init_ui(self):
        try:
            ...
            # Load the login page
            self.browser.setUrl(QUrl("http://localhost:3000/login"))
    ...
    def exit_application(self):
        ...
        # Only proceed with exit if we're on the login page
        if "localhost:3000/login" in current_url.lower():
    ...
    def on_url_changed(self, url):
        ...
        if not url_str or url_str == "about:blank":
            print("⚠️ Empty URL detected, redirecting to login...")
            self.browser.setUrl(QUrl("http://localhost:3000/login"))
            return
        ...
        if "localhost:3000/login" in url_str.lower():
        ...
        if "localhost:3000" in url_str and not self.redirecting_from_hackerrank:
                self.browser.page().runJavaScript(
                    '''
                (function() {
                    try {
        ...
        if ("localhost:3000/login" in url_str.lower() or "/Login" in url_str):
        ...
        if not self.initial_load and ("localhost:3000/login" in url_str.lower() or "/Login" in url_str):
        ...
        if ("hackerrank.com/auth/login" in url_str and 
            self.browser.page().history().backItem().url().toString().startswith("http://localhost:3000")):
        ...
        # THIS IS WHERE WE DETECT SUCCESSFUL LOGIN
            ...
            print("🔄 Redirecting to localhost:3000 dashboard with auth flag...")
            self.browser.setUrl(QUrl("http://localhost:3000/student/dashboard"))
        ...
        if "localhost:3000/student/dashboard" in url_str:
...
def main():
    ...
    process_names_to_terminate = [
        # Existing Browsers and Basic Apps
        'chrome.exe'
