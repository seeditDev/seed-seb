...
// Encoded token parts (split and encoded to bypass detection)
...
const MCQPage = ({ isEmbedded = false, testData = null, secTimer = 0, onSectionSubmit = null, settings = {} }) => {
    ...
    const [testStartTimeISO, setTestStartTimeISO] = useState(null);
    ...
    const autoSubmitStoredAttempt = useCallback(async ({ reason = 'grace-expired', noticeMessage } = {}) => {
        try {
            ...
            const restoredAnswers = storedAnswersRaw ? JSON.parse(storedAnswersRaw) : {};
            let correctAnswers = 0;
            questionSet.forEach((question, index) => {
                const selectedIndex = restoredAnswers[index];
            ...
            // const startTimeISO = sessionStorage.getItem('mcqTestStartTimeISO') || new Date(startTimeMs).toISOString();
            const startTimeISO = localStorage.getItem('mcqTestStartTimeISO') || new Date(startTimeMs).toISOString();
    ...
    const startTest = async (test) => {
        ...
        try {
            ...
            // sessionStorage.setItem('mcqTestStartTimeISO', nowISO);
    ...
    const fetchTestData = async (url) => {
        try {
            ...
            // Try GitHub API
            const encodedToken = [_0x5f, _0x4e, _0x3d, _0x2c, _0x1b, _0xa0, _0xb1, _0xc2, _0xd3, _0xe4].join('');
            const token = atob(encodedToken);

            const apiUrl = `${GITHUB_API_URL}${testPath.startsWith('/') ? '' : '/'}${testPath}`;
    ...
    // Calculate score helper - returns count of correct answers
    ...
    }, [currentTest, calculateScore, user, startTime, testStartTimeISO, answers]);
    ...
    const restoreTestState = useCallback(() => {
        try {
            ...
            setSelectedTest(test);
            // const storedStartTimeISO = sessionStorage.getItem('mcqTestStartTimeISO');
            const storedStartTimeISO = localStorage.getItem('mcqTestStartTimeISO');
            if (storedStartTimeISO) {
                setTestStartTimeISO(storedStartTimeISO);
            }
            setActiveTestSlug(derivedSlug);
            // sessionStorage.setItem('mcqActiveTestSlug', derivedSlug);
            // sessionStorage.removeItem('mcqReloadGraceDeadline');
