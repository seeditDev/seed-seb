    ...
    const handleSaveContest = async (e) => {
        ...
        if (auth.currentUser) {
            try {
                ...
                adminTenantId = adminProfile.data().tenantId || '';
            } catch (profileErr) {
                toast.error(
                    `Failed to read admin profile (${profileErr.message}). ` +
                    'Cannot create assessment key with unknown tenant. Please retry.'
                );
                return;
            }
        } else {
        ...
        // Hard guard: a blank tenantId would orphan the key — reject before writing.
        if (!adminTenantId) {
            toast.error(
        ...
        const keyRef = doc(db, "assessment_keys", contestId);
        let keyProvisionedThisRun = false;

        try {
            // ── Phase 2: Key Generation & Pre-Provisioning ───────────────────────
            // Key is generated and verified BEFORE publishing/saving the contest doc
            // so an assessment document is never created without its authoritative key.
            ...
            try {
                await setDoc(doc(db, "contests", contestId), {
                    ...
                    tenantId: adminTenantId,
                    keyProvisioned: true,
                    status: contestForm.status || 'published',
                    updatedAt: new Date().toISOString(),
                });
            } catch (contestWriteErr) {
                // Transactional rollback: if contest write fails, wipe newly provisioned key
                if (keyProvisionedThisRun && !editingContestId) {
                    try {
                        await deleteDoc(keyRef);
                    } catch (_) {}
                }
                throw new Error(`Contest record write failed (${contestWriteErr.message}). Provisioned key safely rolled back.`);
            }

            toast.success("Contest validated, encryption key provisioned, and published successfully.");
