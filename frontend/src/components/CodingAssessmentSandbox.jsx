...
const CodingAssessmentSandbox = ({ isEmbedded = false, testData = null, secTimer = 0, onSectionSubmit = null, settings = {} }) => {
    ...
    const triggerAutoSubmit = async (finalCode, finalLang, challengeId, contestId) => {
        ...
        try {
            ...
            await setDoc(doc(db, "users", user.Email, "codingAttempts", challengeId), {
                completed: false,
                language: finalLang,
            ...
            if (contestId) {
                ...
                await setDoc(doc(db, "users", user.Email, "contestAttempts", contestId), {
                    status: "completed",
                    tabViolation: true,
                    timestamp: serverTimestamp(),
    ...
    const handleManualSubmit = async () => {
        ...
        try {
            ...
            try {
                if (selectedChallenge) {
                    await setDoc(doc(db, "users", user.Email, "codingAttempts", selectedChallenge.id), {
                ...
                await setDoc(doc(db, "users", user.Email, "contestAttempts", contestParam), {
                    status: "completed",
                    timestamp: serverTimestamp(),
