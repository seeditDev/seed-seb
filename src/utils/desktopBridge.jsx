...
const desktopBridge = {
    setStudentSession: async (authData) => {
        ...
        if (backend) {
            // [P2] SECURITY: Verify that the uid in authData matches the currently
            // authenticated Firebase user before forwarding to the native bridge.
            // Firestore rules enforce server-side access control, but the bridge
            // uses uid for local file-path scoping.  A mismatch here would cause
            // student A's local data to be written under student B's uid.
            const currentUser = auth?.currentUser;
            if (!currentUser) {
                console.error('[DesktopBridge] SECURITY: setStudentSession refused — no authenticated Firebase user. ' +
                    'Sign in before calling setStudentSession.');
                return;
            }
            if (currentUser.uid !== authData?.uid) {
                console.error(
                    '[DesktopBridge] SECURITY: setStudentSession refused — uid mismatch. ' +
                    `authData.uid=${authData?.uid} does not match auth.currentUser.uid=${currentUser.uid}. ` +
                    'This indicates a bug in the caller. Session NOT forwarded to native bridge.'
                );
                return;
            }
    ...
    setContestContextWithQuestions: async (contestId, keyHex = "", questionIds = []) => {
        const backend = await initBridge();
        if (backend && typeof backend.setContestContextWithQuestions === 'function') {
            const questionIdsJson = JSON.stringify(Array.isArray(questionIds) ? questionIds : []);
            return await backend.setContestContextWithQuestions(String(contestId ?? ""), String(keyHex ?? ""), questionIdsJson);
        }
        return false;
    },

    markAssessmentSubmitted: async () => {
        const backend = await initBridge();
        if (backend && typeof backend.markAssessmentSubmitted === 'function') {
            return await backend.markAssessmentSubmitted();
        }
        return false;
    },
    ...
    endAssessment: async () => {
        const backend = await initBridge();
        if (backend && typeof backend.endAssessment === 'function') {
            return await backend.endAssessment();
        }
        return false;
    },

    forceEndAssessment: async () => {
        const backend = await initBridge();
        if (backend && typeof backend.forceEndAssessment === 'function') {
            return await backend.forceEndAssessment();
        }
        return false;
    },
    ...
    /**
     * fetchContestKey — Explicit credentialed key pull.
     *
     * Asks the Python engine to attempt a Firebase-authenticated Firestore
     * pull of assessment_keys/{contestId} using the ID token stored in the
     * current student session.
     *
     * Intended for admin/staff launch flows where the caller's Firebase
     * session has permission to read assessment_keys/.  Student sessions will
     * receive a 403 (admin-only rules) — that is expected and logged; in that
     * case the key must have been delivered earlier via setContestKey().
     *
     * @param {string} contestId
     * @returns {Promise<{ok: boolean, reason?: string}|null>}
     */
    fetchContestKey: async (contestId) => {
        const backend = await initBridge();
        if (backend && typeof backend.fetchContestKey === 'function') {
            const raw = await backend.fetchContestKey(String(contestId ?? ""));
    ...
    runCode: async (language, code, stdin = "") => {
        ...
        // ── Piston remote execution gate ─────────────────────────────────────
        // In desktop/SEB mode (window.qt present) we must NEVER fall back to a
        // remote execution service. Student code must stay on the local machine.
        // Reaching this point means the local engine failed; return the same
        // ENGINE_NOT_CONNECTED signal as runDirectSandbox() so the UI can inform
        // the student to restart the application.
        if (isDesktopEnvironment()) {
            console.error("[DesktopBridge] runCode: desktop engine unavailable. Remote execution blocked in SEB mode.");
            notifyEngineDisconnected();
            return {
                stdout: "",
                stderr: "Evaluation engine not connected. Please restart SEED SEB to reconnect the coding engine.",
                output: "",
                exit_code: -1,
        ...
        // Practice mode / Web environment — remote execution via Piston API is acceptable
        try {
